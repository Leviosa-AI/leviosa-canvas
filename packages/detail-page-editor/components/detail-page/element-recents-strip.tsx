...
export function ElementRecentsStrip({ store }: { store: unknown }) {
  ...
  return (
    <div className="shrink-0 border-b border-le-ink-200 px-2 py-2">
      <p className="mb-1.5 px-0.5 text-[11px] font-le-medium text-le-ink-500">
        ...
        {items.map(({ entry, isPinned }) => (
          <div key={entry.key} className="group relative shrink-0">
            <button
              ...
              className="flex h-11 w-11 items-center justify-center rounded-le-lg border border-le-ink-200 bg-le-surface p-1.5 hover:border-le-ink-400"
            >
            ...
            <button
              ...
              className={`absolute -right-1 -top-1 rounded-full border border-le-ink-200 bg-le-surface p-0.5 transition-opacity ${
                isPinned
                  ? "text-le-warn-500 opacity-100"
                  : "text-le-ink-300 opacity-0 group-hover:opacity-100"
              }`}
            >

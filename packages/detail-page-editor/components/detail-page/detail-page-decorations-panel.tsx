...
export function DetailPageDecorationsPanel({ store }: { store: unknown }) {
  ...
  if (isError) {
    return (
      <div className="p-3">
        <p className="text-xs text-le-danger-600">{t("detailPage.shapes.decorationsFailed")}</p>
      </div>
    );
  }
  ...
  if (isPending) {
    return (
      <div className="flex h-full items-center justify-center text-le-ink-400">
  ...
  if (!groups.length) {
    return (
      <div className="p-3">
        <p className="text-xs text-le-ink-400">{t("detailPage.shapes.decorationsEmpty")}</p>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-4 gap-2">
            {cells.map(({ item, uri }) => (
              <button
                ...
                type="button"
                onClick={() => insert(item)}
                className="flex aspect-square items-center justify-center rounded-le-lg border border-le-ink-200 p-2 hover:border-le-ink-400"

    ...
    <button
      ...
      className={[
        ...
        disabled
          ...
          : "hover:bg-le-ink-200/70 hover:text-le-ink-800",
      ].join(" ")}
    >
      {children}
    </button>
  ...
  return (
    <>
      <div
        ...
        className={[
          "group relative flex items-center gap-1 rounded-le-md pr-1.5 text-le-ink-700",
          "min-h-[30px] cursor-default select-none",
          dragging ? "opacity-40" : "",
          over === "inside" ? "ring-2 ring-inset ring-le-select-400" : "",
          selected
            ? "bg-le-select-200 text-le-select-950"
            : context.has(el.id)
              ? "bg-le-select-50 text-le-select-900 hover:bg-le-select-100"
              : "hover:bg-le-ink-100",
        ].join(" ")}
      ...
      >
        {over === "before" || over === "after" ? (
          <span
            ...
            className={[
              "pointer-events-none absolute inset-x-1 h-0.5 rounded-full bg-le-select-500",
        ...
        {isGroup ? (
          <button
            ...
            className="flex h-5 w-5 shrink-0 items-center justify-center rounded text-le-ink-400 hover:bg-le-ink-200/70 hover:text-le-ink-700"
          >
...
export const DetailPageLayersPanel = observer(function DetailPageLayersPanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex items-center justify-between px-3 py-2.5 text-[11px] font-le-semibold uppercase tracking-[0.06em] text-le-ink-400">
        <span>{t("detailPage.layers.title")}</span>
        {/* 머리글 숫자는 **맨 위 층의 줄 수**다. 그룹을 펴면 줄은 늘지만 레이어 수는
            그대로다 — 펼침 상태에 따라 숫자가 오르내리면 그건 개수가 아니다. */}
        <span className="tabular-nums text-le-ink-300">

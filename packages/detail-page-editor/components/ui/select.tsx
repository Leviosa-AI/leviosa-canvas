 ...
 * 구워졌고, 목록 판이 **투명해져** 뒤의 라벨과 글자가 겹쳐 보였다(다운로드 팝오버의
 * "등록 플랫폼" 목록). 팔레트를 좁게 두는 것은 그 앱의 정당한 선택이다 — 없는 이름을
 * 부른 쪽이 잘못이다. command.tsx 가 같은 이유로 이미 옮겨졌다.
 *
 * 호버·선택은 회색 한 단계(`dpe-ink-100`)로 끝내고, 판은 `dpe-surface` 위에
 * `dpe-ink-900` 글자다. dark: 변형은 지웠다 — 편집기에는 어두운 토큰이 없다.
 */
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "border-dpe-ink-200 text-dpe-ink-900 data-[placeholder]:text-dpe-ink-400 [&_svg:not([class*='text-'])]:text-dpe-ink-500 focus-visible:border-dpe-select-500 focus-visible:ring-dpe-select-200 aria-invalid:border-dpe-danger-500 aria-invalid:ring-dpe-danger-100 flex w-fit items-center justify-between gap-2 rounded-dpe-md border bg-transparent px-3 py-2 text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 data-[size=default]:h-9 data-[size=sm]:h-8 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
...
function SelectSeparator({
  ...
  return (
    <SelectPrimitive.Separator
      ...
      className={cn("bg-dpe-ink-100 pointer-events-none -mx-1 my-1 h-px", className)}
      {...props}
    />
  )
}

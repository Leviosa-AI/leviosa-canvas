  ...
  return (
    <div ref={containerRef} className="relative inline-block">
      ...
      <button
        ...
        className={`${swatchPx} rounded-le-lg border-2 border-[var(--color-border)] cursor-pointer shrink-0 transition-all hover:border-[var(--color-primary)] hover:shadow-md hover:scale-105 active:scale-95`}
        style={{
      ...
      {open && popoverPos && createPortal(
        <div
          ref={popoverRef}
          data-layer-panel
          className={`fixed z-[200] w-56 bg-[var(--color-background)] border border-[var(--color-border)] rounded-le-xl shadow-lg p-3 space-y-2.5 transition-opacity duration-75 ${measured ? "opacity-100" : "opacity-0 pointer-events-none"}`}
          ...
          <div className="grid grid-cols-6 gap-1.5">
            {allowTransparent && (
              <button
                ...
                className={`w-7 h-7 rounded-le-md border transition-transform hover:scale-110 active:scale-95 relative overflow-hidden ${
            ...
            {PRESET_COLORS.map((color) => (
              <button
                ...
                className={`w-7 h-7 rounded-le-md border transition-transform hover:scale-110 active:scale-95 ${
              ...
              {Array.from({ length: 8 }, (_, idx) => {
                ...
                return color ? (
                  <button
                    ...
                    type="button"
                    onClick={() => selectColor(color)}
                    className={`size-5 rounded-le-md border transition-transform hover:scale-110 active:scale-95 ${
                      !isTransparentValue && safeColor.toUpperCase() === color ? "border-[var(--color-primary)] ring-1 ring-[var(--color-primary)]" : "border-[var(--color-border)]"
                    }`}
                    style={{ backgroundColor: color }}
                  />
          ...
          <div className="flex items-center gap-1.5 pt-1 border-t border-[var(--color-border)]">
            {"EyeDropper" in globalThis && (
              <button
                ...
                className="flex shrink-0 items-center gap-1 rounded-le-lg px-1.5 py-1.5 text-[11px] text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface)] hover:text-[var(--color-text-primary)]"
            ...
            <input
              ...
              placeholder="#000000"
              className="min-w-0 flex-1 rounded-le-lg border border-[var(--color-border)] px-2 py-1.5 text-xs font-mono focus:outline-none focus:border-[var(--color-primary)]"
            />
          </div>
        </div>,
        document.body
